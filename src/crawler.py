...
async def get_page_content(url, session):
...
async def crawl_forum_page(page_num, session, url_queue):
    page_url = f"{settings.FORUM_URL}page/{page_num}/"
    ...
    for link in thread_links:
        ...
        # Use a bloom-like set for this session to avoid queueing duplicates
...
async def worker(name, url_queue, session):
    ...
    while True:
        try:
            ...
            logger.error(f"Worker {name} caught an exception: {e}")
        finally:
            url_queue.task_done()
            
async def run_crawler(initial_run=False):
    """Main crawler function to be scheduled."""
    from main import url_queue, http_session
    ...
    for page_num in range(1, max_pages + 1):
        if not await crawl_forum_page(page_num, http_session, url_queue):
            break
